...
pub static METRICS: Lazy<Mutex<MobileMetrics>> = Lazy::new(|| {
    Mutex::new(MobileMetrics {
        throughput_mbps: 0.0,
...
///
/// Ownership: the Kotlin side must hand the fd over with
/// `ParcelFileDescriptor.detachFd()` — Rust closes it when the session ends.
