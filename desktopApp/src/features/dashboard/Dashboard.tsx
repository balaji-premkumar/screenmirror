...
export const Dashboard: React.FC<DashboardProps> = ({ startupChecks }) => {
  ...
  // Track consecutive missed polls to debounce flickering
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      ...
      try {
        ...
        if (data && !cancelled) {
          setStatus(prev => {
            // Debounce logic: Only set isConnected to false if we missed multiple polls
            // This prevents flickering if the USB bus is briefly busy
            let nextIsConnected = data.isActive;
            if (!data.isActive) {
                missedPolls.current++;
                if (missedPolls.current < 3 && prev.isConnected) {
                    nextIsConnected = true; // Keep it true for now
                }
            } else {
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section className="bg-[#0e1015] p-6 rounded-2xl border border-gray-800 shadow-2xl">
                ...
                <div className="flex gap-4 mt-8">
                    ...
                    <button 
                        onClick={stopStream}
                        className="flex-1 py-3 bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-500 text-[10px] font-black uppercase rounded-xl border border-yellow-500/20 transition-all active:scale-[0.98] cursor-pointer"
                    >
                        Stop Mobile Capture
                    </button>
                    <button 
